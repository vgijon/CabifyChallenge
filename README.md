...
**The solution is added below as additional sections**
...
The implementation of this service will be done with Python and also with the Django Framework. Django will allow us to do a simple implementation of the Rest API. And also, as one of its advantages, it will allow us to separate the logic of the product from the implementation of the API in a simple way.

### Decisions
As the challenge said, I'm not going to use a database, so, the data will be saved in memory. Therefore, we will lose the data every time we finish the service. It means, this is an example service if we want to continue the project and put it in production we will need to use a database, but that's not a problem because Django allows us to connect to a database. And we will be able to use the ORM of Django to interact with the database!
